				...
				for (int a = -1; a <= 1; ++a)
					for (int b = -1; b <= 1; ++b)
					{
						...
						if (isCellInbound(second_cell_x, second_cell_y))
							if (_field[second_cell_x][second_cell_y] == CROSS) //if it`s true, we have too			|x|_|_|	|x|_|_|
							{												  //check next cell in a row after them	|x|_|_|	|_|X|_|
								...
								if (isCellInbound(second_cell_x + offset_x, second_cell_y + offset_y)) 
									if (TryToPutMark(second_cell_x + offset_x, second_cell_y + offset_y)) //our game winning mark
										return true;
							}
							else if (_field[second_cell_x][second_cell_y] != ZERO) //if the cell near current exist,		 	|x|_|_|	|x|_|_| 
																					...
																					//there can be situation like this:			|x|_|_|	|_|_|x|
							{
								...
								if (isCellInbound(second_cell_x + offset_x, second_cell_y + offset_y)) 
									if (_field[second_cell_x + offset_x][second_cell_y + offset_y] == ZERO) //our game winning mark
									{
